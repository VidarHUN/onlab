...
fogadja és továbbítsa a bejövő forgalmat. Ezt egy egész egyszerű pranacssal
megtudjuk tenni: \textbf{envoy -c <konfig fájl elérési útvonala>}
...
Az iperf szervert a következő parancssal hozzuk létre.
\begin{center}
  \begin {verbatim}
     iperf -s -p 5000 -u -U -e -i 1 --realtime
  \end{verbatim}
\end{center}
Ezzel megadtuk, hogy az 5000-s porton figyejen UDP forgalomra és csak egy UDP
...
általunk beállított proxy. 72.3 Mbit/s sebességel 100 bájtos csomagokkal
...
képfájlt a DockerHub-ról, amit a következő parancssal egyszerűen megtehetünk.
\begin{center}
  \begin{verbatim}
  ...
  \end{verbatim}
\end{center}
Az első parancssal az áltunk kívánt képfájl és adott verziója fog letöltődni,
...
parancssal könnyen ellenőrizni tudjuk. De minden más marad a régiben.
...
jön az izgalmasabb rész, ahol az általunk írt konfigurációs fájl tartalmatá betöltjük a
kontérben lévőbe. Ezt azért kell megcsinálni, mert az alapképfájlban van egy
...
A sebesség mérése csak annyiban fog különbözi az előzőjhöz képest, hogy nem a
...
Ahhoz, hogy kubernetes-ben is létretudjuk hozni ugyan ezt a környezetet, ahhoz kell
...
Minikube-t a következő parancssal.
\begin{verbatim}
  minikube start
\end{verbatim}
...
Ezt könnyen kideríthető az \textbf{ip a} parancssal.
...
funkcionalítású, de más IP-vel rendelkező Pod. De ugyanakkor később, ha több
...
mert a Pod teljes mértékben képes kilátni a klaszteből és a VirtualBox
